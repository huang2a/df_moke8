...
<div id="threadlist" class="tl bm bmw"{if $_G['uid']} style="position: relative;"{/if}>
	...
	<!--{/if}-->
    <style>
	.waterfall .df_subject{height:36px; text-align:center;background-color: #F8F8F8;}
	.waterfall h3{padding:0; height:36px;line-height:36px;overflow:hidden;}.waterfall h3 a{ font-size:14px;line-height:36px;}</style>
	<div class="th">
							...
							<tbody id="$thread[id]"{if $_G['hiddenexists'] && $thread['hidden']} style='display:none'{/if}>
								<tr>
									...
									<th class="$thread[folder]">
										...
										<!--{if $thread[icon] >= 0 && $thread[icon]<=20}-->
