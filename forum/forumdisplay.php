...
<div class="boardnav">
<style>
.tl .waterfall{ margin-left:0px;}

.bm{ background-color:transparent; border:0px; }

.bbs{ background-color:#FFFFFF; border-bottom:0px !important;}
.tl .th{ border-top:1px solid #dcdcdc; margin-top:0px; border-bottom:0px;}
.tl .tf {
padding: 3px 0;
}
...
.tl .tf .showmenu {
padding-right: 20px;
background-image: url(template/df_moke8/images/arrwd.png);
}
...
width: 100px;
height: 35px;
background: url(template/df_moke8/images/pn_post.png) no-repeat 0 0;
}
/*版主*/.df_moderatedby{background-color: #FFF;padding: 0px 20px 5px 20px;font-size: 12px;color: #999;}
</style>
...
<h1><a href="forum.php?mod=forumdisplay&fid=$_G[fid]"  class="indexTitleT f20 yh c666">$_G['forum'][name]</a></h1>
<span class="list_xtit c999 f12">（共有主题: <span class="xi1">$_G[forum][threads]</span> 个）</span>
<!--百度分享-->
<div style=" padding-left:10px; margin-top:2px; width: 200px; display: inline; float: left;">
...
<!--版块banner end-->
		<div class="mn">
			...
			<!--{if !empty($_G['forum']['recommendlist'])}-->
				<!--推荐主题S-->
				<div class="bm bmw">
					<div class="bm_h cl" style="background:#fff;">
					...
					<ul id="thread_types" class="ttp cl" >
						...
						<!--{if $_G['forum']['threadsorts']}-->
							...
							<!--{loop $_G['forum']['threadsorts']['types'] $id $name}-->
								<!--{if $_GET['sortid'] == $id}-->
								<li class="xw1 a"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]{if $_GET['typeid']}&filter=typeid&typeid=$_GET['typeid']{/if}{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}">$name<!--{if $showthreadclasscount[sortid][$id]}--><span class="xg1 num" style="display:none;">$showthreadclasscount[sortid][$id]</span><!--{/if}--></a></li>
								<!--{else}-->
								<li><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=sortid&sortid=$id$forumdisplayadd[sortid]{if $_GET['archiveid']}&archiveid={$_GET['archiveid']}{/if}">$name<!--{if $showthreadclasscount[sortid][$id]}--><span class="xg1 num" style="display:none;">$showthreadclasscount[sortid][$id]</span><!--{/if}--></a></li>
								<!--{/if}-->
							<!--{/loop}-->
						<!--{/if}-->
