...
<!--{loop $post['verifyicon'] $vid}-->
    ...
    <!--{if $_G['setting']['verify'][$vid]['icon']}-->
        <img src="$_G['setting']['verify'][$vid]['icon']" class="vm" alt="$_G['setting']['verify'][$vid][title]" title="$_G['setting']['verify'][$vid][title]" />						    <!--{else}-->
...
<table id="pid$post[pid]" class="plhin" summary="pid$post[pid]" cellspacing="0" cellpadding="0">
  ...
  <tr> 
    ...
    <div id="favatar$post[pid]" class="pls favatar"> $post[newpostanchor] $post[lastpostanchor] 
        ...
        <div > 
          ...
          <!--{elseif $post['avatar'] && $showavatars}--> 
              <!--{if $post[mobiletype]}-->
              <div class="mobile-type mobile-type-$post[mobiletype]"> <a></a> </div>
              <!--{/if}-->
       <div class="avatar"{if !($_G['setting']['threadguestlite'] && !$_G['uid'])} onmouseover="showauthor(this, 'userinfo$post[pid]')"{/If}>$post[avatar]</div>
          <!--{/if}--> 
        ...
        <!--{if !($_G['setting']['threadguestlite'] && !$_G['uid'])}--> 
            ...
            <ul class="xl xl2 o cl" style="display:none;">
              ...
              <li class="addflw"> <a href="home.php?mod=spacecp&ac=follow&op=add&hash={FORMHASH}&fuid=$post[authorid]" id="followmod_$post[authorid]" title="{lang follow}" class="xi2" onclick="showWindow('followmod', this.href, 'get', 0);">{lang follow}</a> </li>
              <!--{/if}-->
        ...
        <div class="pi"> 
          ...
          <!--{elseif $post['authorid'] && $post['username'] && $post['anonymous']}--> 
              <!--{if $_G['forum']['ismoderator']}--><a href="home.php?mod=space&uid=$post[authorid]" target="_blank">$_G[setting][anonymoustext]</a>
        ...
        <!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}--> 
            <!--{if $_G['setting']['authoronleft']}-->
            <div class="pi">
              <div class="authi"> <font class="xw1 " style="color:#4095CE;">$post[author]</font>
                <p  style="text-align:center; margin:0; color:#ccc; font-size:12px;">$post[authortitle]</p>
                $authorverifys </div>
            </div>
            <!--{/if}--> 
  		 <!--{/if}--> 
 ...
 <!--{if ($_G['group']['allowedituser'] || $_G['group']['allowbanuser'] || ($_G['forum']['ismoderator'] && $_G['group']['allowviewip'])) && !getstatus($post['status'], 5)}-->
    ...
    <div class="plcinfo"> 
      ...
      <div class="pbm bbda" style="text-align:center; border-bottom:0px;"> 
 		<h1><a href="forum.php?mod=viewthread&tid=$_G[tid]" id="thread_subject">$_G[forum_thread][subject]</a></h1>
   		<span class="xg1"> 
        ...
        <!--{/if}--> 
        <a href="forum.php?mod=viewthread&tid=$_G[tid]$fromuid" onclick="return copyThreadUrl(this)" {if $fromuid}title="{lang share_url_copy_comment}"{/if}>[{lang share_url_copy}]</a> </span> 
        </div>
      <!--forum_thread subject end--> 
      <!--{/if}-->
        ...
        <div class="pi"<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}--> style="height:48px"<!--{/if}-->> 
          ...
          <!--{if !IS_ROBOT}--> 
              ...
              <!--{/if}--> 
          <strong {if $post['first']} style="display:none;"{/if}><a href="{if $post[first]}forum.php?mod=viewthread&tid=$_G[tid]$fromuid{else}forum.php?mod=redirect&goto=findpost&ptid=$_G[tid]&pid=$post[pid]$fromuid{/if}"  {if $fromuid}title="{lang share_url_copy_comment}"{/if} id="postnum$post[pid]" onclick="setCopy(this.href, '{lang post_copied}');return false;"> 
      ...
      <div class="pti">
        ...
        <div class="authi xg1" {if $post['first']}style="text-align:center;{/if}"> 
          ...
          <!--{if $_self }-->$post[author]<!--{else}-->$post[author] &nbsp;<!--{/if}--> 
          ...
          <em id="authorposton$post[pid]">{lang poston} $post[dateline]</em> 
              <span class="pipe">|</span>栏目：<a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=sortid&sortid=$_G[forum_thread][sortid]">{$_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']]}</a> 
              
          <!--{if $post['first']}--> 
              
              <!--{if $_G['forum_thread']['typeid'] && $_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}--> 
                  <!--{if !IS_ROBOT && ($_G['forum']['threadtypes']['listable'] || $_G['forum']['status'] == 3)}--> 
                  <span class="pipe">|</span>分类：<a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=typeid&typeid=$_G[forum_thread][typeid]">{$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}</a> 
                  <!--{else}--> 
                  <span class="pipe">|</span>分类：{$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]} 
                  <!--{/if}--> 
              <!--{/if}--> 
...
.pcb{margin-right:0}
    ...
    <div style="font-size:14px"> 
      <!--广告-->
      <p style="font-size:12px; color:#999999; border:1px solid #dcdcdc; background-color:#f9f9f9; padding:6px 10px; margin-bottom:20px;">说明：本站所有资源仅供学习与参考，请勿用于商业用途，否则产生的一切后果将由您自己承担！如有侵犯您的版权，请及时联系我们，我们将尽快处理。 </p>
      <!--广告end--> 
      
    </div>
    <div class="moke8_thread_bottom cl"> 
      ...
      <p  class="like_btn">
        ...
        <em><i><span id="recommendv_add">$_G[forum_thread][recommend_add]</span></i>$_G['setting']['recommendthread'][addtext]</em>
          </a>
      </p>
      <!--{/if}--> 
      ...
      <p class="favo_btn"> <a href="javascript:;" id="k_favorite" onclick="showWindow(this.id, 'home.php?mod=spacecp&ac=favorite&type=thread&id=$_G[tid]&formhash={FORMHASH}', 'get', 0);" onmouseover="this.title = $('favoritenumber').innerHTML + ' {lang activity_member_unit}{lang thread_favorite}'" title="{lang fav_thread}"><em><i><!--{if !$_G['forum_thread']['favtimes']}-->0<!--{else}-->{$_G['forum_thread']['favtimes']}<!--{/if}--></i>收藏</em> </a></p>
      <p class="repo_btn"> <a href="#postlistreply" class="smooth" onclick="return false;"><em><i>$_G[forum_thread][allreplies]</i>回复</em></a> </p>
      <p class="view_btn"> <a href="#pt" class="smooth" onclick="return false;"><em><i>$_G[forum_thread][views]</i>浏览</em> </a> </p>
    </div>
...
jq(".smooth").click(function(){
    ...
    <div class="plc" style="overflow:visible;{if $close_leftinfo}--> width:100%{/if}">
      <div class="po hin"> 
...
{lang support_reply}           <span id="review_support_$post[pid]">$post[postreview][support]</span>
            </a>
...
{lang against_reply}           <span id="review_against_$post[pid]">$post[postreview][against]</span>
            </a>
          <!--{/if}--> 
          ...
          <p> 
            ...
            <!--{if $post['first']}--> 
            <a  href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&reppost=$post[pid]&extra=$_GET[extra]&page=$page" onclick="showWindow('reply', this.href)">{lang reply}</a> 
            <!--{else}--> 
            <a  href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&repquote=$post[pid]&extra=$_GET[extra]&page=$page" onclick="showWindow('reply', this.href)">{lang reply}</a> 
            <!--{/if}--> 
            <!--{/if}--> 
            <!--{if $post['invisible'] == 0}--> 
            ...
            <!--{if $post['first'] && helper_access::check_module('share')}--> 
            <a  href="home.php?mod=spacecp&ac=share&type=thread&id=$_G[tid]" onclick="showWindow('sharethread', this.href, 'get', 0);" title="{lang share_digest}">{lang thread_share}<!--{if $_G['forum_thread']['sharetimes']}--><span id="sharenumber">{$_G['forum_thread']['sharetimes']}</span><!--{/if}--></a> 
            <!--{/if}--> 
            
            <!--{if $post['first'] && !$_G['forum']['disablecollect'] && helper_access::check_module('collection')}--> 
            <a href="forum.php?mod=collection&action=edit&op=addthread&tid=$_G[tid]" id="k_collect" onclick="showWindow(this.id, this.href);return false;" onmouseover="this.title = $('collectionnumber').innerHTML + ' {lang activity_member_unit}{lang collection}'" title="{lang thread_collect}">{lang collection}<span id="collectionnumber"{if !$post['releatcollectionnum']} style="display:none"{/if}>{$post['releatcollectionnum']}</span></a> 
            <!--{/if}--> 
            <!--转播分享掏贴 end--> 
  ...
  <!--{/if}-->
  <tr class="ad">
    <td class="pls"><!--{if !$close_leftinfo && !$post[first]}--></td>
    <td class="plc"><!--{/if}--> 
      ...
      <!--{/if}--> 
      <!--{if $_G['forum_thread']['replies']}--><!--{ad/interthread/a_p/$postcount}--><!--{/if}--></td>
  </tr>
</table>
